...
def cornersHeuristic(state, problem, goal):
    ...
    # a position and whether or not we have visited it
    combined = [(manhattan(coords, corners[i]), corners[i], visited[i]) for i in range(0, len(visited))]
    
    # sort based on distance to the coordinate
    combined_sorted = sorted(combined, key=lambda i: i[0])
